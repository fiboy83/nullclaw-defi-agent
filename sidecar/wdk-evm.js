...
/**
 * Initialize EVM wallet from seed phrase
 */
export async function initWallet(seedPhrase) {
  ...
  try {
    ...
    // Derive primary account at path m/44'/60'/0'/0/0
...
export async function lend({ action, token, amount }) {
  ...
  try {
    ...
    switch (action) {
      case 'supply':
        result = await lendingProtocol.supply({
          asset: tokenAddress,
          amount: parsedAmount,
        });
        break;
      case 'withdraw':
        result = await lendingProtocol.withdraw({
          asset: tokenAddress,
          amount: parsedAmount,
        });
        break;
      case 'borrow':
        result = await lendingProtocol.borrow({
          asset: tokenAddress,
          amount: parsedAmount,
        });
        break;
      case 'repay':
        result = await lendingProtocol.repay({
          asset: tokenAddress,
          amount: parsedAmount,
        });
        break;
...
export async function bridge({ toChainId, amount }) {
  ...
  try {
    const parsedAmount = parseBalance(amount, 6); // USDT has 6 decimals
